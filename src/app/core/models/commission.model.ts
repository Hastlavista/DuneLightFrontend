...
/** A correction (see BookingService.ApplyIndividualCompletionCorrection on the
 * backend) flips the ORIGINAL historical CommissionEntry row Earned ->
 * Reversed rather than deleting it, and a later re-completion creates a brand
 * new Earned row - both stay visible in history as distinct occurrences, never
 * merged/deduplicated by bookingId client-side. */
const COMMISSION_ENTRY_STATUS_SEVERITIES: Record<CommissionEntryStatus, 'success' | 'danger'> = {
  Earned: 'success',
  Reversed: 'danger',
};

export function commissionEntryStatusSeverity(status: CommissionEntryStatus): 'success' | 'danger' {
  return COMMISSION_ENTRY_STATUS_SEVERITIES[status];
}

const COMMISSION_ENTRY_STATUS_LABELS: Record<CommissionEntryStatus, string> = {
  Earned: 'Zarađena',
  Reversed: 'Poništena',
};

export function commissionEntryStatusLabel(status: CommissionEntryStatus): string {
  return COMMISSION_ENTRY_STATUS_LABELS[status];
}
